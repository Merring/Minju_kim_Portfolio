$(document).ready(function(){
    ...
    */
    ...
    for(i=0; i<works_list.length; i++){
        $(".slide_frame").append(works_cont);
    };

    for(i=0; i<works_list.length; i++){
        $(".cont_info").append(works_txt);
    };

    $("#works .slides").each(function(i){
        $(this).find("li").css("background-image", "url(./img/works_banner/"+works_list[i][0]+")");
    });
    $("#works .text_box .cont_info").each(function(i){
        $(this).find(".works_title").text(works_list[i][1]);
        $(this).find(".works_context").text(works_list[i][2]);
    });
    */
